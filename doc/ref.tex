...
persistant information.  Unordered tasks run on any available slave and
...
exploring game search trees, webcrawling, and monte-carlo simulations.
...
structure given back to the \ma which contains the echo'ed string
\texttt{"Hello World"}.
...
sequential slaves. These sequential tasks will be run in the
order inserted by the \ma.  The \tp is directly responsible for a
sequential task possibly becoming \emph{unrunnable}. This happens when
the sequential slave which is processing the task disconnects. The
default \tp for sequential tasks is that any tasks being processed on
...
without having to have all of them in existance at once. The tasks
...
itself when creating tasks. The master is free to inject more tasks
...
{Function}
	...
	return zero if the return value of \func{mw-get-target-nuber-for-echo} 
...
\item Acquiring and managing sequential slaves
...
or disconnection of sequential slaves, etc) only when there is some
...
The \sa is an optional portion of a \clmw application. It is
often a very simple call to \func{mw-slave-loop-simple} which will
wait for tasks to arrive from the master, process them, send the
results back, and continue as such until the master sends a shutdown
command. \func{mw-slave-loop-simple} returns 0 if the shutdown was
normal and explicitly from the master.
...
job, the master can still determine the truename of the specified
