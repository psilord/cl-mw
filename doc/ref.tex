...
high throughput for tens of thousands of jobs on tens of thousands
...
The particular interface described in this section is a simple one
and doesn't make full use of the information the \rfile. It could
be extended with an actual process outside of \clmw and managed by
Condor which watches the \rfile and actively submits and removes
...
job, which states to only let the job be removed from the queue if
it had not exited with a signal and the exit code was zero.
