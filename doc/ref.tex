...
then \bold{use-package}s it, and then calls \func{mu-dump-exec}
to produce the binary. 

\func{mw-dump-exec} will copy any currently loaded libraries
with an absolute path into the current working directory.
\func{mu-dump-exec} will update the Lisp image to look for the
dumped libraries in the path \file{./} when the saved executable
is started.  Libraries with relative paths are ignored by default;
they are typically widely available system libraries.  This simplifies
collecting required libraries that may not be present on the slave
system.
...
\clmw) must be included and the actual library file is copied into
the current working directory. Then the Lisp image is adjusted to
look in the path \file{./} for \file{libio-syscalls.so}.  The library
\file{librt.so} didn't have an absolute path and, by default, won't
be copied. If \func{mw-dump-exec} isn't told otherwise, the name of
the binary it dumps is \file{a.out}. You can supply a different executable
name to \func{mw-dump-exec}, see page~\pageref{mw-dump-exec} for details.
